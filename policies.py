...
class LogisticPolicy(Policy):
	...
	def fit(self, ratings):
		...
		for i in range(ratings.shape[0]):
			...
			if (reward != 0):
				self.p_visit += 1
				self.update(context, action, reward, None, None)
			## ignore if reward = 0
	...
	def update(self, context, rec_items, reward, div_intra, div_inter):
		...
		arm = self.f_mix(rec_items, context)
		self.arms.append(arm.ravel())
		self.rewards.append((reward+1)/2)
...
class Custom(LogisticPolicy):
	def __init__(self, info, alpha=1., max_steps=5, lazy_update_fr=5, random_state=1234):
	...
	def predict(self, context, k, only_available=False):
		available_items_ids = np.zeros(context.shape[0], dtype=int) #get_available_actions(context)
		...
		qis[qis<=0] = np.min(qis)/2      ## positive scores
...
class LinUCBDiversity(LinUCB):
	...
	def fit(self, ratings):
		...
		for i in range(ratings.shape[0]):
			...
			available_items_ids = np.zeros(context.shape) #get_available_actions(context)
			if (available_items_ids.sum()==0):
				#print(f"All items are explored for user {context_array2int(context, 1)}")
				available_items_ids = np.ones(available_items_ids.shape, dtype=int)
